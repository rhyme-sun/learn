...
如果从对 Spring IoC 容器进行扩展这个角度出发，那么好的方式就是利用 Spring IoC 容器提供的扩展点，也就是 `BeanFactoryPostProcessor` 和 `BeanPostProcessor`。因为 AOP 是基于动态代理实现的，需要为每个实例创建代理类，所以核心的扩展点应该可以用 `BeanPostProcessor` 实现。实际上 Spring 正式这么做的，对应的 `BPP` 为 `AbstractAutoProxyCreator`。

除此之外，还需要我们根据需求进行 AOP 编程，比如定义切面，定义切入点，实现通知逻辑等。

### 功能开启

从使用 Spring AOP 的角度去将，首先要为 Spring 开启 AOP 支持，这里有两种方式，基于 XML 和基于配置类。
...
编写 AOP 处理逻辑。比如这里我们想在执行某个 bean 的方法时，在其运行前后加入一些日志打印能力。这些日志打印能力的编写我们可以同一放到一个类中，和业务代码独立开。
...
`<aop:config>` 是和 AOP 处理相关的核心配置。声明了切面类为 `logUtil`，切面里有包含了切点的定义和通知处理定义。
...
通过 Spring IoC 的启动流程，我们可以知道。`ClassPathXmlApplicationContext` 再启动过程中，会去获取 `BeanFactory`，并在这个时候会去解析配置元信息。上述编写的 XML 文件就是这个时候解析的。那么上述配置文件解析后会产生那些内容呢？

这里先给出结论，再来看这些对象是怎么生成的。

上述的 XML 文件在解析后，会注册以下 bean 定义：
...
> 通过调用 `AopConfigUtils#registerAspectJAnnotationAutoProxyCreatorIfNecessary(BeanDefinitionRegistry)`

接下来解析 `<aop:config>` 元素，对应解析器为 `ConfigBeanDefinitionParser`。其主要用来解析切面配置，注册切点和通知 bean 定义信息。

如果不开启 `<aop:aspectj-autoproxy/>`，在解析  `<aop:config>`  也会注册 `org.springframework.aop.config.internalAutoProxyCreator`，不过类型为
...
实例化的时机和位置比较隐蔽，位置在 `AnnotationAwareAspectJAutoProxyCreator#findCandidateAdvisors`。实际为实例化第一个 bean 时，在实例化前通过调用 `InstantiationAwareBeanPostProcessor#postProcessBeforeInstantiation` 执行，调用下面代码中的 `shouldSkip` 时触发。
...
在讨论 Spring IoC 基于注解的上下文容器启动过程知道，和注解相关配置元信息的解析是通过 `ConfigurationClassPostProcessor`  这个 `BFPP` 来实现的。其实和注解相关的功能实现，都离不开它，它是一个很核心的一个类。
...
> ```
>
> 比如 A 和 B 相互依赖，创建 A 设置属性 B 的过程中会创建 B，B 在填充其 A 属性时会调用三级缓存中的回调方法将 A 的引用放到 `earlyProxyReferences` 中。可以看到上述回调方法也会调用 `wrapIfNecessary`，也就是 A 被提前增强了。
>
...
代码开始的三个判断命中时，就好直接跳过后面代理类的创建过程。

第一个判断是当前 bean 是自定义目标 bean，表明已经创建过了，直接跳过。

第二个判断表明，当前的 bean 是否在被通知的范围内，如果不在，直接跳过。

带三个判断表明，当前类如果是一个基础设施类，或者应该被跳过则直接返回，并在 `advisedBeans` 缓存结果。

这里的 `shouldSkip` 判断逻辑如下：
...
考虑使用 JDK 动态代理或 CGLIB，生成代理类的过程，关键的步骤在于定义并添加增强方法处理，比如 JDK 的 `InvocationHandler`，`CGLIB` 的 `Callback`。

Spring 的处理过程应该是类似的。这里思考一个问题，前期创建好的通知 bean 是怎么和动态代理的增强处理关联起来的。
...
这里关注上上面的 `getCallbacks` 方法，其会创建一个 `DynamicAdvisedInterceptor`，将通知 bean 包装起来（`AdvisedSupport`），并包含了通知类的调用逻辑。
...
this 本事作为一个 `InvocationHandler`，其实现了调用通知类的逻辑（`AdvisedSupport`）。
...
> 如果是 `GGLIB` 动态代理类，处理器是 `DynamicAdvisedInterceptor` ，最终也会执行 `ReflectiveMethodInvocation#proceed` 方法。
...
4. 0 号拦截器为 `ExposeInvocationInterceptor`，他主要是将当前方法执行对象（`ReflectiveMethodInvocation`）设置到线程局部变量中，然后调用  `ReflectiveMethodInvocation#proceed` ，又回到了步骤 2；
...
5. 步骤 2 选择接着选择 1 号拦截器， `AspectJAfterThrowingAdvice`，执行 `invoke` 方法，方法如下：
   ...
   发现其会先调用了 `proceed`  方法，等调用结束如果出现异常，执行异常处理逻辑，因此会先再次回到步骤 2：
...
中间穿插了 `ReflectiveMethodInvocation#proceed` 来控制拦截器链的执行。

先调用不代表会限制通知逻辑，实际上根据上面的执行流程我们可以得出，在通过动态代理类执行方法时，通知逻辑执行的顺序为：
