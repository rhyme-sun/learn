...
- 准备 0 ~ 9 一共 10 个桶；
- 从左到右遍历数据，将数据放入个位数对应的桶中，然后桶中依次弹出（先进先出），这样数据顺序就变为 `[090, 012, 122, 034, 056, 006]`；
- 接着和上一步一样，不过此时根据是为数进行操作，数据顺序就变为 `[006, 012, 122, 034, 056, 090]`；
...
- 将 count 数组变成程斐波那契数列，即从左往右用前一个数和本身相加的数替换本身，这样一来 count 数组的含义就变为：
  - count[0] 表示个位数 <=0 的数字的个数；
  - count[1] 表示个位数 <=1 的数字的个数；
  - ....
  - count[9] 表示个位数 <=9 的数字的个数；

- 从右往左遍历数组中的数据，获取个位上的数字，假设为 5，取出 count[5] 的值假设为 6，则表明数组中个位数 <=5 的数有 6 个，又因为是从右到左排序，所以这个数字应该放到数组的 5 位置，并且将 count[5] 减一，这样遍历完毕后数据的排列顺序就和上面使用桶进行第一次排序的结果是一致的；
