...
public class StringComparator {
     ...
     * @return 随机字符串
     */
    public static String generateLowercaseRandomString(int strLen) {
        char[] ans = new char[(int) (Math.random() * strLen) + 1];
        for (int i = 0; i < ans.length; i++) {
            int value = (int) (Math.random() * 5);
            ans[i] =  (char) (97 + value);
     ...
     * @return 随机字符串数组
     */
    public static String[] generateLowercaseRandomStringArray(int arrLen, int strLen) {
        String[] ans = new String[(int) (Math.random() * arrLen) + 1];
        for (int i = 0; i < ans.length; i++) {
            ans[i] = generateLowercaseRandomString(strLen);
        }
        return ans;
    }
