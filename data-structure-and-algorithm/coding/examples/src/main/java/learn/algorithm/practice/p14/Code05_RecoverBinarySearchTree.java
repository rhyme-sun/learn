...
public class Code05_RecoverBinarySearchTree {
    ...
    // 使用栈实现非递归中序遍历
    private static TreeNode[] twoErrors2(TreeNode head) {
        TreeNode cur = head;
        ...
        return new TreeNode[]{firstPair, lastPair};
    }

    // Morris 遍历实现中序遍历，找到两个交换的节点
    private static TreeNode[] twoErrors3(TreeNode head) {
        TreeNode cur = head;
        TreeNode pre = null;
        TreeNode firstPair = null;
        TreeNode lastPair = null;
    ...
    // 交换节点
    static TreeNode recoverTree2(TreeNode head) {
