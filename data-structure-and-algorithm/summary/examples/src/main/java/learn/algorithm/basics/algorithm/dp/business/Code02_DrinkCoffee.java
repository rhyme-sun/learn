 ...
 */
public class Code02_DrinkCoffee {
    ...
    static int[] bestSchedule(int[] arr, int n) {
        // 小根堆，按照咖啡机可用时间点和工作时间和排序
        PriorityQueue<Machine> heap = new PriorityQueue<>(Comparator.comparing(
                (Machine m) -> m.timePoint + m.workTime));
        for (int i = 0; i < arr.length; i++) {
