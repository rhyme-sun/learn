...
[VLOOKUP函数最常用的10种用法，你会几种？ - 简书 (jianshu.com)](https://www.jianshu.com/p/0a12fcabbfaa)
...
[一个例子拉通LEFT、MID、RIGHT、FIND、LEN五个Excel函数用法，你总会用到 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/395624125)

LEFT 函数的主要作用是基于指定的字符串返回文本里字符串中从**左开始的**第一个或前几个字符串：

> LEFT(text, [num_chars])
> LEFT(要提取字符的文本字符串, [由 LEFT 所提取的字符数])

RIGHT 函数的主要作用是基于指定的字符串返回文本里字符串中**从右侧开始**的最后一个或后几个字符串：

> RIGHT(text,[num_chars])
> RIGHT(要提取字符的文本字符串, [由 RIGHT 所提取的字符数])

MID 函数的主要作用是从一个字符串中截取出指定数量的字符：

> MID(text, start_num, num_chars)
> MID(要提取字符的文本字符串，从左起第几位开始截取，截取几位字符)

LEN 函数用于返回文本字符串中的字符个数：
...
`400*500*600`（位于 B3 单元格）

截取长：`=LEFT(B3,FIND("*",B3)-1)`

截取宽：`=MID(B3,FIND("*",B3)+1,FIND("*",B3,FIND("*",B3)+1)-1-FIND("*",B3))`

截取高：`=RIGHT(B3,(LEN(B3)-FIND("*",B3,FIND("*",B3)+1)))`



### OFFSET

[【Excel技巧】- 办公常用的十大函数@OFFSET函数 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/26558367)
...
OFFSET(基准单元格, 纵向偏移, 横向偏移, 纵向连续偏移数量, 横向连续偏移数量) 

结合数组公式（CSE）实现多个单元格克隆。



OFFSET 使用场景广泛：

[如何在Excel中使用OFFSET函数？ (extendoffice.com)](https://zh-cn.extendoffice.com/excel/functions/excel-offset-function.html)



### COUNT

[COUNT 函数 (microsoft.com)](https://support.microsoft.com/zh-cn/office/count-函数-a59cd7fc-b623-4d93-87a4-d23bf411294c)



COUNT 统计单元格内数字的个数。

[COUNTA](https://support.microsoft.com/zh-cn/office/counta-%E5%87%BD%E6%95%B0-7dc98875-d5c1-46f1-9a82-53f3219e2509) 计算某个范围内非空单元格的个数。

[COUNTIF](https://support.microsoft.com/zh-cn/office/countif-%E5%87%BD%E6%95%B0-e0de10c6-f885-4e71-abb4-1f464816df34) 计算符合某个标准的单元格的个数。
...
- 表格模板行内单元格相互引用
  ...
  在表格的模板行的单元格中使用公式，可引用模板行中的单元格，此时会在表格的整列应用这个公式，且每一行的公式都会**引用同行**的区域。也就是说同一个表格中的模板行引用运行时只能作用于同一行，不能同时作用于多行。
...
一般都是使用当前表格的同行的字段作为参数，在辅助表格的区域中使用相关公式获得一个值，并填充到当前表格的同行的某个字段中。例如，在订单系统中，订单详情表为订单表的子表，在订单详情表中有订单的总金额字段，可通过公式获取到订单详情表中的总金额字段，将其显示在订单表中。具体见下面操作。

例如，在订单系统中，订单详情表为订单表的子表，在订单详情表中有订单的总金额字段，可通过公式获取到订单详情表中的总金额字段，将其显示在订单表中。
...
[看完必恍然大悟的活字格内幕：二，使用OData日期，结果不对 - 活字格专区 - 专题教程 - 葡萄城产品技术社区 (grapecity.com.cn)](https://gcdn.grapecity.com.cn/forum.php?mod=viewthread&tid=38101&extra=page%3D4%26filter%3Dtypeid%26typeid%3D242)

OData 查询时间时需要转换成文本格式（自动转换），可以看上述链接。
...
一个单元格如果既有绑定又有公式，绑定优先级高，但是如果公式引用的单元格修改了，发送了改变，就会触发公式的重算。**此外还需要注意一点是，因当前行变更和绑定字段引起引用单元格的值变更，不会触发 Excel 公式（前提是 Excel 公式和绑定字段同时使用）**。

OData 公式和绑定字段同时作用一个单元格规则和上述一致。

来自：

- [活字格 ODATA 和 公式的处理逻辑 - 活字格专区 - 求助中心 - 葡萄城产品技术社区 (grapecity.com.cn)](https://gcdn.grapecity.com.cn/showtopic-89201-1-1.html)

- [看完必恍然大悟的活字格内幕：二十八、公式不计算，可能是绑定字段在捣蛋 - 活字格专区 - 专题教程 - 葡萄城产品技术社区 (grapecity.com.cn)](https://gcdn.grapecity.com.cn/forum.php?mod=viewthread&tid=56268&fromuid=52377)
