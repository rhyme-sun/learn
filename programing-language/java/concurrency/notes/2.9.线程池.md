...
Java 中使用 Thread 对象来操作线程，每个线程和一个 Thread 实例对象关联。下图展示了一个 Java 线程的创建过程：

![](../images/Java线程创建过程.png)

可以看出，实际上创建线程远不是创建一个对象那么简单。创建一个线程，最终需要调用操作系统的 API，然后操作系统要为线程分配一系列的资源，这个成本就很高了。总的来说，线程是一个重量级的对象，应该避免频繁创建和销毁。
...
- `int maximumPoolSize` 线程总数最大值
  ...
  > 核心线程和非核心线程只是逻辑上的概念，对线程池来说都是工作线程，只会当前线程池的工作线程数来进行调整，并没有使用额外的状态区分。
...
- `Executors.newScheduledThreadPool`
  ...
  > 使用线程池我们应该注意什么？
  >
  > Java 并发包里提供了一个线程池的静态工厂类 Executors，利用 Executors 你可以快速创建线程池，不过目前阿里的编码规范中基本上都不建议使用 Executors 了，因为 Executors 提供的很多方法默认使用的都是无界的 LinkedBlockingQueue，高负载情境下，无界队列很容易导致 OOM，而 OOM 会导致所有请求都无法处理，这是致命问题，所以强烈建议在使用线程池时使用有界队列。
  >
