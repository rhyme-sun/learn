...
Linux 命令查看推荐：[linux命令在线中文手册 (51yip.com)](http://linux.51yip.com/)。
...
- 将远程主机的文件拷贝到本地：
  ...
  ```shell
  ssh -r 192.168.0.18:/root/test1.txt /root/test2.txt
  ```

- 将本地主机的文件拷贝到远程：

  ```shell
  ssh -r /root/test1.txt 192.168.0.18:/root/test2.txt
  ```
